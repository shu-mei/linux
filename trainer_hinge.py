...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss, ckp, writer=None, converging=False, model_teacher=None):
        ...
        # self.scheduler= <misc.warm_multi_step_lr.HingeMultiStepLR object at 0x7fe492d9cf10>
    ...
    # def compute_scatter_matrix(self, img, label):
    #     class_num = torch.max(label) + 1
    #     b, c, w, h = img.size()
    #     u_i = torch.zeros(class_num, c, w, h)
    #     u_i = u_i.to(self.device)
    #     u = torch.zeros((1, c, w, h))
    #     u = u.to(self.device)
    #     N_i = torch.zeros(class_num)
    #     N_i = N_i.to(self.device)
    #     N = torch.zeros(1)
    #     N = N.to(self.device)
    ...
    #     u[0, :, :, :] = u[0, :, :, :] / N[0]
    #
    #     s_w = torch.zeros((1, c, w, h))
    #     s_w = s_w.to(self.device)
    #     s_w = torch.reshape(s_w, (1, c * w * h))
    #     u_i = torch.reshape(u_i, (class_num, c * w * h))
    #     img = torch.reshape(img, (b, c * w * h))
    #     s_b = torch.zeros((1, c, w, h))
    #     s_b = s_b.to(self.device)
    #     s_b = torch.reshape(s_b, (1, c * w * h))
    #     u = torch.reshape(u, (1, c * w * h))
    #     for index, i in enumerate(label):
    #         s_w[0, :] += torch.mul((u_i[i, :] - img[index, :]), (u_i[i, :] - img[index, :]).t())
    #     for i in range(class_num):
    #         s_b[0, :] += N_i[i] * torch.mul((u_i[i, :] - u[0, :]), (u_i[i, :] - u[0, :]).t())
    #     return s_b, s_w
    ...
    def train(self):
        ...
        for batch, (img, label) in enumerate(self.loader_train):
            ...
            # s_b, s_w = compute_scatter_matrix(self, img, label)
            class_num = torch.max(label) + 1
            b, c, w, h = img.size()
            u_i = torch.zeros(class_num, c, w, h)
            u_i = u_i.to(self.device)
            ...
            lossp = self.model.get_model().compute_loss(batch + 1, epoch, self.converging)  # hinge_vgg.py
    ...
    def start_epoch(self):
        ...
        # print('epoch=', epoch)
        # print('self.scheduler.last_epoch=', self.scheduler.last_epoch)
        # lr= [0.001, 0.1]
