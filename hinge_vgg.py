...
def set_module_param(module, params):
    ...
    # Note that the bias term is added to the second conv(注意，偏差项被添加到第二个conv中)
    # Do not need to set batchnorm1, activation, and batchnorm2.(不需要设置batchnorm1、activation和batchnorm2)
    # embed()
...
def modify_network(net_current):
    ...
    # skip the first BasicBlock that deals with the input images(跳过处理输入图像的第一个基本块)
    for module_cur in modules[1:]:
        # embed()
        # get initialization values for the Block to be compressed(获取要压缩的块的初始化值)
        ...
        # modify submodules in the ResBlock(修改ResBlock中的子模块)
        modify_submodules(module_cur)
        # set ResBlock module params(设置ResBlock模块参数)
...
class Hinge(VGG):
    ...
    def compute_loss(self, batch, current_epoch, converging):
        ...
        for l, m in enumerate(modules):
            ...
            # TEMP1=projection1.squeeze().t() ** 2
            # TEMP2=torch.sum(projection1.squeeze().t() ** 2, dim=1)
            # TEMP3=torch.sum(projection1.squeeze().t() ** 2, dim=0)
            # TEMP4=torch.sum(torch.sum(projection1.squeeze().t() ** 2, dim=1) ** (q / 2))
            # TEMP5=torch.sum(torch.sum(projection1.squeeze().t() ** 2, dim=0) ** (q / 2))
